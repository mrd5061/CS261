...
int _indexSmallest(DynamicArray* v, int i, int j, compareFunction compare) {
	/* return index of smallest element */
	assert(i <= dySize(v));
	assert(j <= dySize(v));
	if (compare(dyGet(v, i), dyGet(v, j)) == -1)
		return i;
	else
		return j;
}
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	if (rightChild < last) {
		...
		lesserChild = _indexSmallest(heap, leftChild, rightChild, compare);
		if (compare(dyGet(heap, position), dyGet(heap, lesserChild)) == 1) {
			dySwap(heap, position, lesserChild);
			adjustHeap(heap, last, lesserChild, compare);
		}
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
	...
	i = heap->size - 1;
	parentIDX = (i - 1) / 2;
	void* parentVal = heap->data[parentIDX];

	while (i > 0 && compare(value, parentVal) == -1) {
		dySwap(heap, i, parentIDX);
		i = parentIDX;
		parentIDX = (i - 1) / 2;
...
{
	heap->data[0] = dyGet(heap, (dySize(heap) - 1));
	heap->size--;
